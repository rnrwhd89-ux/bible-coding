...
// CSV 파일에서 창세기 1~4장만 업로드
async function uploadMissingGenesis() {
  const results = [];
  ...
  fs.createReadStream('./bible.csv')
    ...
    .on('data', (data) => {
      ...
      // 창세기 1~4장만 필터링
      if (cleanData.book === '창' && parseInt(cleanData.chapter) >= 1 && parseInt(cleanData.chapter) <= 4) {
        results.push(cleanData);
      }
    })
    .on('end', async () => {
      console.log(`창세기 1~4장 총 ${results.length}개의 구절을 읽었습니다.`);
      ...
      console.log(`✅ 창세기 1~4장 총 ${totalCount}개의 구절이 성공적으로 업로드되었습니다!`);
      process.exit(0);
    });
}

uploadMissingGenesis().catch(console.error);
